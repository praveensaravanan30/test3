"""
Pytest test runner for grading generated testbenches with assertions.

This test runner:
1. Checks if testbench has assertions
2. Compiles the testbench with iverilog
3. Runs simulation with vvp
4. Verifies assertions were executed
5. Grades the testbench

Note: This is a SystemVerilog/iverilog-based test, NOT cocotb.
The agent generates a SystemVerilog testbench that is compiled and run with iverilog/vvp.
"""
...
def test_testbench_compiles(test):
    """
    Test that generated testbench compiles successfully with iverilog.
    """
    if not testbench_exists():
        pytest.skip(f"Testbench file not found: {testbench_path}")
...
def test_testbench_simulates(test):
    """
    Test that generated testbench simulates successfully with vvp.
    """
    if not testbench_exists():
        pytest.skip(f"Testbench file not found: {testbench_path}")
...
# ✅ REQUIRED: Pytest wrapper function for HUD framework
# Note: This is NOT using cocotb_tools.runner because we use pure SystemVerilog/iverilog
# The wrapper just ensures pytest can discover and run the test file
def test_axi4_slave_tb_hidden_runner():
    """
    Pytest wrapper function required by HUD framework.
    
    For SystemVerilog/iverilog-based tests (NOT cocotb):
    - All test functions above are already pytest test functions
    - Pytest will automatically discover and run them
    - This wrapper just ensures the file is discoverable and verifies setup
    
    The actual test execution happens via:
    1. Agent generates SystemVerilog testbench in verif/axi4_slave_tb.sv
    2. Tests use subprocess to run: iverilog -g2012 sources/axi4_slave.sv verif/axi4_slave_tb.sv
    3. Tests run simulation: vvp sim_build/tb.out
    4. Tests check simulation logs for assertions
    """
    import os
    from pathlib import Path
    
    # Verify sources exist (DUT files)
    ...
    missing_sources = []
    for src_file in required_sources:
        src_path = sources_dir / src_file
        if not src_path.exists():
            missing_sources.append(str(src_path))
    
    if missing_sources:
        pytest.fail(f"Required source files not found: {', '.join(missing_sources)}")
    
    # Verify verif directory exists (where agent writes testbench)
    verif_dir = proj_path / "verif"
    if not verif_dir.exists():
        # This is okay - agent will create it
        pass
    
    # All test functions above will be run by pytest automatically
    # This wrapper just ensures the test file structure is valid
    # No need to actually run anything here - pytest handles test discovery
