...
# This is required for HUD framework to discover and run the tests
def test_axi4_slave_tb_hidden_runner():
    """
    Pytest wrapper function required by HUD framework.
    This ensures pytest can discover and run the test file.
    """
    import os
    from pathlib import Path
    ...
    for src_file in required_sources:
        ...
        assert src_path.exists(), f"Required source file not found: {src_path}"
    
    # The actual tests are defined above as pytest test functions
    # This wrapper just ensures the test file is discoverable
